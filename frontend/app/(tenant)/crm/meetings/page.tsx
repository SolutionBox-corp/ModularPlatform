...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t("page.meetingsMetaTitle") };
}
...
export default async function CrmMeetingsPage() {
  ...
  void queryClient.prefetchQuery(crmQueries.meetings({ page: 1, pageSize: 20 }));

  return (
    <HydrationBoundary state={dehydrate(queryClient)}>
      <div className="space-y-6">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">{t("meetings.pageHeading")}</h1>
          <p className="text-sm text-muted-foreground mt-0.5">{t("meetings.pageDescription")}</p>
        </div>
