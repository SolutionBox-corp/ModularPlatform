# ModularPlatform pro novy produktovy modul
...
## 11. Dosazeni pro CRM jako priklad

Kdyz opravdu stavis CRM, dosadis:

| Placeholder | CRM dosazeni |
|---|---|
| `{ModuleName}` | `Crm` |
| `{module}` | `crm` |
| `{Record}` | `Contact`, `Company`, `Deal`, `Activity` |
| `{RecordAttachment}` | `DealAttachment` |
| `{PaidActionRun}` | `CrmAiRun` |
| permission | `crm.read`, `crm.write` |
| route | `/crm/contacts`, `/crm/deals`, `/crm/ai/...` |
| frontend | `frontend/features/crm/*` |

Ale architektura neni CRM-specific. CRM je jen jedna instance stejneho modularniho vzoru.
