...
Katalog má degradační UC pro graf (17-02), embed (17-03), rerank (17-04), generate (17-05) i all-down (17-06), ale CHYBÍ dedikovaná noha pro LEXIKÁLNÍ/BM25 nohu — přitom je v `legs[]` jako jeden z 5 zdrojů (EC-17-01-03) a má vnitřní fallback pg_search→ts_rank_cd, který je TICHÁ změna kvality řazení.

### UC-17-15 — Lexikální/BM25 noha down nebo degradovaná (pg_search→ts_rank_cd) → fail-loud, ne tichá půlka
- **EC-17-15-01 — ParadeDB/pg_search nedostupný → ts_rank_cd fallback je SILENT quality drop** · Trigger: pg_search (`@@@`) extension chybí/selže, handler spadne na `ts_rank_cd` LINQ fallback · Očekávané chování: ts_rank_cd NENÍ BM25 (jiné skóre) → přechod je kvalitní degradace a MUSÍ se projevit jako leg příznak (`reason="lexical_fallback_tsrank"`, příp. Partial), ne tichý průchod jako plnohodnotný BM25; zákon „NIKDY tichá půlka" + FAIL-LOUD · Mechanismus: leg nese sub-stav BM25 vs fallback; degraded counter tag `leg=lexical,mode=tsrank`; vazba na frozen „BM25 default, ts_rank_cd fallback, raw-SQL carve-out" · Severity: P1 · Test: vypni pg_search → odpověď přijde z ts_rank_cd, ale `legs.lexical` nese fallback reason + příznak nižší kvality.
- **EC-17-15-02 — Lexikální noha úplně down → fallback na vektor+graf (Partial)** · Trigger: i ts_rank_cd selže (DB chyba na FTS) · Očekávané chování: ladder pokračuje s vektor+graf kandidáty, `legs.lexical.ok=false reason="lexical_unavailable"`, Status≥Partial, sanitizovaný reason (ne raw SQL/exception — EC-17-02-05) · Mechanismus: typed `RetrievalLegException(source="lexical")`, degraded counter `leg=lexical` · Severity: P1 · Test: shoď FTS, assert Partial + lexical reason v allowlistu.
- **EC-17-15-03 — Raw-SQL carve-out (`FromSqlInterpolated`) injection guard pod degradací** · Trigger: BM25 carve-out je jediná raw-SQL cesta; query s SQL meta-znaky · Očekávané chování: vždy parametrizovaný `FromSqlInterpolated` (frozen carve-out), žádná string-konkatenace ani ve fallbacku; injection nemožná i při degradaci · Mechanismus: zákon EF/LINQ-only + 3 carve-outy parametrizované · Severity: P1 · Test: query `' OR 1=1; DROP` → bezpečně parametrizováno, žádný efekt.
