...
- **EC-16-07-08 — BM25/lexikální plaintext index drží tokeny Content i po crypto-shred (ZNÁMÝ BLOCKER — OPEN decision, NE tiše vyřešeno)** · Trigger: `Chunk.Content` je `[Encrypted][PersonalData]` at-rest, ale frozen decision vyžaduje pravý BM25 přes pg_search (`@@@`), který indexuje PLAINTEXT tokeny; po GDPR erasure se DEK shredne a Content→`[erased]`, ALE invertovaný BM25/ParadeDB index (a `ts_rank_cd` tsvector fallback) dál obsahuje původní termíny → erased PII je nadále vyhledatelná/rekonstruovatelná přes lexikální nohu · Očekávané chování: TENTO ROZPOR MUSÍ být explicitní otevřené rozhodnutí (Law 11 — ASK), ne tichý předpoklad. Možnosti: (i) lexikální index nad maskovaným/ne-PII textem; (ii) per-tenant/per-subject šifrovaný index (degraduje BM25); (iii) erasure musí REBUILD/DROP lexikální index pro subjektovy chunky (ne jen Content blank); jinak je crypto-shred záruka děravá stejně jako embedding inversion (EC-16-07-01) · Mechanismus: UC-16-07 `RagPersonalDataEraser` musí pokrýt i lexikální index; vazba na frozen „Lexical = pravý BM25 přes pg_search" vs `[Encrypted]` Content · Severity: P0 · Test: erase subjektu → lexikální (BM25 i ts_rank_cd) dotaz na jeho dřívější PII termín → 0 hitů a žádná rekonstrukce z indexu.
- **EC-16-01-10 — pgvector HNSW + RLS/`WHERE` post-filtering pod-vrací k (filtered-ANN recall)** · Trigger: HNSW index je postaven nad VŠEMI řádky napříč scope; `.Where(Scope/Tenant/Owner)` + RLS se aplikuje jako filtr během/po index scanu, ne „před ANN" jak tvrdí EC-16-01-02 — u silně selektivního filtru HNSW vrátí < k vlastních řádků (klasický pgvector post-filter problém), takže recall tiše klesne nebo vyjde prázdno i když relevantní vlastní chunky existují · Očekávané chování: žádný tichý propad recall — použít iterative scan (pgvector 0.8 `hnsw.iterative_scan`) nebo exact-scan fallback při nedostatku kandidátů; výsledek nesmí být méně než k jen kvůli filtru, dokud existují kvalifikované řádky; bezpečnost (0 cross-tenant) zůstává, ale correctness/recall je samostatná záruka · Mechanismus: konfig iterative scan + over-fetch a re-filtr; vazba na frozen „Vektor = pgvector HNSW CosineDistance LINQ" · Severity: P1 · Test: tenant T1 má málo chunků v korpusu zaplaveném T2 vektory bližšími query → search jako T1 vrátí VŠECHNY relevantní T1 chunky (ne pod-výběr kvůli HNSW post-filtru).

### UC-16-11 — PII egress do externích AI sub-processorů (OpenAI / Cohere / Anthropic) — OPEN decision
- **EC-16-11-01 — Plaintext PII odchází na třetí stranu při retrievalu/syntéze** · Trigger: query (může nést PII, např. „plat Jana Nováka") → OpenAI embed; retrieved `[Encrypted][PersonalData]` chunky se dešifrují a posílají na Cohere rerank a na Claude pro syntézu · Očekávané chování: tok PII mimo hranici k sub-processorům musí být VĚDOMÉ rozhodnutí (sub-processor DPA, regionální routing, retence u providera, opt-out / on-prem rerank pro PII korpusy) — at-rest šifrování (UC-16-07) PII nechrání in-transit-to-processor; dnes katalog řeší jen „PII ne do logů/telemetrie" (UC-16-06), ne egress k providerům · Mechanismus: Law 11 ASK — zdokumentovat sub-processor seznam + konfig pro citlivé korpusy; vazba na `Rag:UseFakeGateways` / provider porty · Severity: P1 · Test: dokumentace egressu existuje; konfig „no external rerank/embed for PII collection" respektován (chunk content neopustí proces).
- **EC-16-11-02 — Cache/replay PII u providera po erasure** · Trigger: provider (Anthropic prompt cache, OpenAI) drží PII po TTL i po GDPR erase subjektu · Očekávané chování: TTL-bounded residual akceptován a zdokumentován NEBO PII držena mimo cachovatelnou část (viz EC-14-01-08); žádný neohraničený externí PII state · Mechanismus: bounded lifetime analog zákona „Durable-envelope PII bound" · Severity: P1 · Test: po erase žádný nový egress; max residual TTL dokumentován.


---

## Doplňky / Opravy z PDF audit (PDF §8 — multi-tenancy / production)

### Doplňky k UC-16-01 (per-tenant pre-filtr) — GUC transaction-scope + FORCE RLS
- **EC-16-01-11 — GUC `app.tenant_id` nastaven přes `SET` (session-scoped) místo `SET LOCAL` → přetečení na pooled connection (cross-tenant leak)** · Trigger: `PrincipalSessionConnectionInterceptor` nastaví tenant GUC příkazem `SET app.tenant_id = '<T1>'` (session-level) na connection z Npgsql poolu; po dokončení requestu se connection vrátí do poolu BEZ resetu a další request tenanta T2 ji dostane se stále nastaveným T1 → RLS policy filtruje na cizí tenant · Očekávané chování: tenant/principal GUC MUSÍ být transaction-scoped (`SET LOCAL app.tenant_id = ...` uvnitř transakce každého dotazu) NEBO explicitně přepsaný/resetovaný na začátku KAŽDÉHO requestu PŘED prvním dotazem; nikdy se nesmí spolehnout na to, že příští request GUC přepíše (čistě read-only request bez explicitního setu by zdědil cizí hodnotu) · Mechanismus: +Multi-tenancy/RLS; Npgsql connection pooling + Postgres `SET LOCAL` (transaction GUC) nebo reset-on-return (`DISCARD`); vazba na frozen dual-layer RLS keyed `app.tenant_id` · Severity: P0 · Test: integration — T1 request projde connection a vrátí ji do poolu; T2 čistě read request dostane TUTÉŽ connection (pool size 1) → assert 0 řádků T1 (GUC není zděděný).
- **EC-16-01-12 — RLS jen `ENABLE` bez `FORCE ROW LEVEL SECURITY` → vlastník tabulky obejde policy (tichý cross-tenant leak)** · Trigger: `RlsBootstrapper` na `chunks`/`graph_nodes`/`documents` spustí jen `ALTER TABLE ... ENABLE ROW LEVEL SECURITY`; aplikační runtime role tabulku VLASTNÍ (migrace ji vytvořila pod toutéž rolí) → Postgres pro vlastníka tabulky policy NEvynucuje (owner bypass) → veškerý retrieval běží bez RLS filtru · Očekávané chování: každá chráněná tabulka MUSÍ mít `ENABLE` I `FORCE ROW LEVEL SECURITY` (`FORCE` vynutí policy i pro vlastníka); admin/owner role pro migrace musí mít izolovaný `BYPASSRLS` (CONVENTIONS RLS deployment), ne table-ownership escape · Mechanismus: +Multi-tenancy/RLS; Postgres `FORCE ROW LEVEL SECURITY`; cross-link 0-core UC-CORE-17-03 + CONVENTIONS „FORCE ROW LEVEL SECURITY" · Severity: P0 · Test: integration — připoj se jako vlastnící app role, seed T2 chunk, search jako T1 → assert 0 T2 řádků (s pouhým ENABLE by owner viděl vše); assert `pg_class.relforcerowsecurity=true` na chráněných tabulkách.

### Doplněk k UC-16-02 (per-user privacy) — principal_id GUC leak
- **EC-16-02-07 — GUC `app.principal_id` session-scoped → cross-user leak na pooled connection** · Trigger: stejný pooling problém jako EC-16-01-11, ale pro per-user GUC — user A dokončí request, connection se vrátí do poolu s `app.principal_id=A`, user B (stejný tenant) ji dostane → B vidí A-privátní `IUserOwned` chunky/graph nodes · Očekávané chování: `app.principal_id` nastaven `SET LOCAL` v transakci nebo přepsán/resetován per request před prvním dotazem; fail-closed když GUC chybí (EC-16-02-02 zůstává) · Mechanismus: +Multi-tenancy/RLS; `SET LOCAL` / connection reset-on-return; vazba na `IUserOwned` RLS keyed `app.principal_id` · Severity: P0 · Test: integration — user A request → connection do poolu; user B read na téže connection → assert 0 A-privátních řádků.

### Poznámka (PII egress přes platform klíče)
- **Poznámka (UC-16-04 / UC-16-11 ↔ 0-core UC-CORE-16): PLATFORM provider klíče, ne per-tenant BYOK.** HybridRag volá OpenAI/Cohere/Anthropic JEDNÍM platform API klíčem pro všechny tenanty — viz nový EC-CORE-16-07. Blast-radius: kompromitace platform klíče zasáhne všechny tenanty; provider-side rate-limit + spend jsou sdílené (jeden tenant může vyčerpat kvótu/limit ostatním — mitigace per-tenant budget, ale provider 429 zůstává sdílený); PII všech tenantů teče k sub-procesorům pod jednou identitou (cross-link UC-16-11 PII egress). Vědomé rozhodnutí; per-tenant BYOK = OTEVŘENÉ (EC-CORE-16-06). · Mechanismus: +Multi-tenancy; cross-link 0-core UC-CORE-16.
