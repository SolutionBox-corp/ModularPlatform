...
## Doplňky / Opravy z PDF audit (PDF §1 GraphRAG)

### UC-10-01 (schema-guided extrakce)
- **EC-10-01-16 — Pre-flight ODHAD nákladů + „index sample first / dry-run" před plnou extrakcí velkého korpusu** · Trigger: admin spustí KG extrakci nad kolekcí s desítkami–stovkami tisíc chunků; každý chunk = 1 Claude volání (UC-10-01, jedna zpráva/chunk) → desítky–stovky tisíc LLM volání naslepo; EC-10-01-15 zavádí runtime rozpočtový cap, ale NEdává PŘEDběžný odhad ani možnost ověřit kvalitu na vzorku před plným během · Očekávané chování: před plnou extrakcí systém poskytne pre-flight ODHAD nákladů (počet chunků × průměrné tokeny `prefix+Content` × cena modelu, přes `Microsoft.ML.Tokenizers` count) a nabídne „dry-run / sample first" — extrahovat reprezentativní vzorek (např. 1 dokument / N chunků), změřit reálný náklad i kvalitu grafu, a teprve PO explicitním potvrzení spustit plný běh. Admin rebuild/extract endpoint (202 operace) vrací odhad + sample výsledek, NIKDY rovnou nespustí statisíce volání · Mechanismus: cost estimator (tokenizer × pricing) jako krok admin commandu; sample mód v `IngestSaga`; konzistentní s rozpočtovým capem EC-10-01-15 a zákonem „External System Integration — náklad rozpočtuj od dne 1, ne objevuj incidenty" · Severity: P1 · Test: integrační — extract nad velkou kolekcí nejdřív vrátí odhad nákladů + sample graf; bez potvrzení 0 plných volání; po potvrzení běh respektuje per-document/collection cap (EC-10-01-15).
