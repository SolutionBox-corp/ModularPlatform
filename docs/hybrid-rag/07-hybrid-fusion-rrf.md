...
### UC-07-01 (RRF fúze) — volba metody
- **EC-07-01-08 — RRF vs. min-max normalizace skóre (proč je RRF default)** · Trigger: úvaha „proč nesčítat min-max normalizovaná raw skóre (cosine, BM25) místo ranků" · Očekávané chování: DEFAULT je RRF (rank-only), protože je škálo-invariantní a ODOLNÝ proti outlier BM25 hitu — jeden extrémně vysoký BM25 skóre by po min-max stlačil zbytek lexikálního listu k nule a zkreslil fúzi; RRF řeší jen pořadí, takže outlier neovlivní relativní příspěvky. Min-max normalizace je LEGITIMNÍ alternativa POUZE když explicitně záleží na MAGNITUDĚ relevance (ne jen pořadí) a skóre jsou kalibrovaná/ohraničená — pak je to vědomé tuning rozhodnutí za config flagem, NE default, a NIKDY ne míchání raw (nenormalizovaných) skóre (to je bug, EC-07-01-01) · Mechanismus: frozen default RRF k=60; min-max jako dokumentovaná opt-in alternativa (`Rag:Fusion:Strategy`), kalibrace per retriever; zákon „RRF v C#, raw-mix = bug" · Severity: P2 · Test: unit — list s jedním outlier BM25 skóre: RRF pořadí stabilní, naivní raw-min-max-mix dá zkreslené pořadí; min-max varianta (pokud zapnuta) je deterministická a vyžaduje kalibrovaná ohraničená skóre.

### UC-07-02 (sparse union) — protějšek UC-06-05
- **EC-07-02-05 — Synonymní dotaz: chunk jen v dense listu (BM25 mine) musí přežít union** · Trigger: parafráze/synonymum („vrácení zboží" vs „reklamace", EC-05-01-12) → chunk je v dense listu (rank r), v lexikálním listu CHYBÍ · Očekávané chování: full-outer union chunk zachová s příspěvkem `1/(k+r)` jen z dense (EC-07-02-01 zakazuje INNER JOIN); je to zrcadlový protějšek UC-06-05 (exact-term unikát, kde BM25 trefí a dense mine) — OBA komplementární unikáty musí projít fúzí. Tichý INNER-JOIN by sémantické (synonymní) unikáty zahodil → propad recall na parafrázovaných dotazech · Mechanismus: union klíčů obou listů, chybění v listu = 0 příspěvek (ne penalty, ne `1/(k+∞)`); zákon „RRF recall přes komplementární retrievery" · Severity: P1 · Test: unit — dense-only synonymní chunk je ve výstupu se score přesně `1/(k+r)`, žádný lexikální člen; assert že INNER-JOIN implementace by ho ztratila.
