...
- **EC-29-02-07 — Locale desetinné parsování number/slider (cs čárka)** · Trigger: cs locale, uživatel zadá `0,7` do číselného knobu (threshold / RRF váha) · Očekávané chování: hodnota se naparsuje deterministicky a odešle správně (0.7), NE silent NaN ani špatná hodnota; zobrazení i odeslání konzistentní s backend formátem · Mechanismus: battle-tested number parsing + i18n (žádné ruční `parseFloat` na locale stringu) · Severity: P1 · Test: cs `0,7` → PUT odešle 0.7, ne NaN/7.
- **EC-29-07-06 — Unsaved-changes navigation guard v prompt editoru** · Trigger: reviewer upraví system-prompt template a naviguje pryč / zavře tab bez „Save as new version" · Očekávané chování: guard „Zahodit neuložené změny?" před route change i `beforeunload`; rozdělaný text se neztratí tiše · Mechanismus: dirty-state guard (Surrounding Concerns — no data loss) · Severity: P1 · Test: edit + navigace → potvrzovací dialog, text zachován při Cancel.
- **EC-29-10-06 — Optimistic odebrání z fronty + backend 500 (rollback)** · Trigger: merge/approve/label optimisticky odebere položku, ale backend vrátí 500 (ne 409 already-resolved) · Očekávané chování: položka se vrátí zpět do fronty (rollback), error toast, auto-advance zrušen; reviewer položku neztratí ani ji nepovažuje za vyřešenou (odlišeno od EC-29-10-01) · Mechanismus: optimistic + rollback (Surrounding Concerns #8) · Severity: P1 · Test: mock 500 na merge → položka zpět ve frontě, ne zmizelá.
