...
- **EC-23-05-10 — Smíšené embedding modely v kolekci během/po částečném reindexu → nesmyslná CosineDistance, tichá špatná relevance** · Trigger: kolekce reindexována na nový model po dávkách (UC-23-05 per-run cap / EC-23-05-05) nebo `onlyStale` doběhl jen zčásti; část chunků má vektory starého modelu, část nového; dotaz se embedne JEDNÍM modelem a počítá CosineDistance proti oběma sadám — vzdálenost mezi vektory různých modelů je sémanticky bezvýznamná → tiše degradovaná relevance bez flagu. · Očekávané chování: dotaz nesmí míchat vektory napříč embedding modely; buď kolekce „mid-migration" vrací explicit `Degraded`/blokuje vektorovou větev na nekonzistentní sadě, nebo vektorová větev filtruje na `EmbeddingModel == currentQueryModel` a chybějící označí k reindexu (EC-23-03-05) · Mechanismus: chunk nese `EmbeddingModel`; vektorová větev filtruje na shodný model + explicit degradace (zákon „nikdy tichá půlka"); kolekce drží flag rozpracované migrace · Severity: P1 · Test: kolekce 50/50 starý/nový model → dotaz buď jen shodný model + `Degraded=true`, nebo blok; nikdy tichá CosineDistance napříč modely.
