...
- **EC-30-07-05 — Answer/embed cache jako PII-persistence kanál přežívající erasure** · Trigger: exact/semantic cache (Redis/pgvector) uloží vygenerovanou odpověď nebo prompt, které nesou PII z tenantova korpusu; subjekt pak požádá o GDPR erasure (oblast 20) a DEK se crypto-shredne. · Očekávané chování: cache NESMÍ být kanál, kterým PII přežije erasure — buď je cache namespace tenant+subject-scoped a invaliduje se při erasure subjektu, nebo má krátké TTL (bound jako durable-envelope PII), nebo se cachují jen ne-PII odpovědi; nikdy plaintext PII v Redisu bez bound lifetime. Otevřená rozhodovací díra analogická CONVENTIONS §12 #1 (ta řeší jen lexical/graf index, NE answer cache). · Mechanismus: +oblast 20 (crypto-shred) + CLAUDE.md „Durable-envelope PII bound" (TTL/MAXLEN) + tenant-scoped klíč (oblast 16); rozhodnout v §12 open decisions. · Severity: P0 · Test: integration — odpověď s PII se nacachuje → erasure subjektu → cache záznam invalidovaný/`[erased]`, ne plaintext v Redisu.
- **EC-30-07-06 — Semantic-cache embed volání samo spotřebovává budget / přidává cost na každý dotaz** · Trigger: vrstva 2 (semantic) počítá embedding promptu přes `gateway.EmbedAsync`, které prochází routing→budget→usage (UC-30-01); při téměř vyčerpaném budgetu embed pro cache-lookup spadne na 429 → cache se vůbec nezkusí a levný hit se nikdy nedosáhne; navíc cost+latence embedu na KAŽDÉM dotazu i při následném hitu. · Očekávané chování: embed pro semantic-cache je odlehčená přímá cesta, která nepodléhá hard-budget 429 (jinak by budget zablokoval i cache lookup), je levně účtovaná `Feature=cache_probe` a nerekurzí zpět do cache vrstev. · Mechanismus: +UC-30-01/UC-30-05 (budget guard) — cache-probe embed mimo hard-stop; oblast 22 Redis. · Severity: P1 · Test: integration — tenant nad budgetem → exact/semantic lookup stále proběhne (cache-probe embed neselže na 429); embed pro cache se nezacyklí.
- **EC-30-05-05 — Orphan budget rezervace po crashi mezi reserve a reconcile** · Trigger: gateway atomicky rezervuje cost odhad v Redis bucketu (UC-30-05 krok 1), pak proces/worker spadne PŘED reconcile (krok 3) → rezervace se nikdy nevrátí. · Očekávané chování: rezervace nesmí trvale nafouknout bucket a předčasně zamknout tenanta — rezervace má vlastní TTL/expiraci nebo periodický reconcile sweep vrací osiřelé rezervace; bucket konverguje ke skutečné spotřebě z ledgeru. · Mechanismus: +oblast 22 (Redis bucket TTL) + reconcile vzor (CLAUDE.md external-system reconciliation / oblast 19 rollup jako pravda) — rezervace jako Billing pending s expirací. · Severity: P1 · Test: integration — rezervuj → kill před reconcile → po TTL/sweep je bucket vyčištěn, tenant není trvale zablokován.
- **EC-30-03-04 — Idempotency `RequestId` bez tenant scope → cross-tenant dedup kolize a ztracený náklad** · Trigger: ledger UNIQUE je na holém `RequestId`, ale idempotency key je odvozen z obsahu (např. `embed:{documentId}:{ingestRunId}:{chunkHash}` dle CONVENTIONS §5); dva tenanti s identickým obsahem dokumentu → stejný `chunkHash` → stejný key → druhý INSERT dedup'nut. · Očekávané chování: usage druhého tenanta NESMÍ být dedup'nut proti prvnímu — UNIQUE klíč je `(TenantId, RequestId)` (nebo RequestId nese tenant), jinak se cizí náklad tiše ztratí a hrozí i cross-tenant kolize. · Mechanismus: +Zákon 3/10 (tenant izolace) + idempotency UNIQUE key musí zahrnovat `tenant_id`; oblast 16. · Severity: P1 · Test: integration — 2 tenanti, identický content-hash request → 2 ledger řádky (každý svůj tenant), žádný dedup napříč tenanty.
- **EC-30-09-04 — Confidence cascade dvojitá budget rezervace → eskalace 429 v půlce dotazu** · Trigger: cascade (UC-30-09) volá cheap model (rezervuje+reconciliuje) a pak eskaluje na frontier (druhá rezervace); tenant je u limitu → druhá rezervace narazí na budget → frontier volání 429, uživatel dostane jen slabou cheap odpověď bez signalizace. · Očekávané chování: cascade počítá s cost obou tierů konzistentně; eskalace blokovaná budgetem nevrací tiše degradovanou cheap odpověď jako plnohodnotnou — buď reservace pro cascade počítá frontier headroom dopředu, nebo se odpověď označí degraded (`RetrievalStatus`/answer metadata), ne falešně „kvalitní". · Mechanismus: +UC-30-05 budget + oblast 17 degradace; cascade jako jedna logická jednotka cost. · Severity: P1 · Test: integration — tenant těsně pod limitem → hard dotaz → eskalace 429 → odpověď nese degraded flag, ne tichý cheap výstup.
- **EC-30-12-04 — Stale routing/pricing cache po registry změně (ztracený invalidační event)** · Trigger: `RagModelUpdatedIntegrationEvent` (disable/pricing change) se ztratí/zpozdí → gateway drží starou in-memory routing/pricing cache → routuje na deprecated/disabled model nebo účtuje starou cenu. · Očekávané chování: cache má bounded TTL (ne jen event-driven invalidaci) a/nebo verifikuje status modelu při výběru; ztráta jednoho eventu nezpůsobí trvale špatné routování/cenu; disabled model se nikdy nepoužije i při stale cache. · Mechanismus: +CLAUDE.md „NO global ordering / handlers idempotent+order-independent" (event může zaostat) — TTL fallback + status re-check; oblast 24 config precedence. · Severity: P1 · Test: integration — disable model + zahoď invalidační event → po TTL/při výběru gateway model nepoužije, fallback na default.
- **EC-30-12-05 — Deklarovaná `EmbedDimensions` ≠ skutečný rozměr vektoru z provideru za běhu** · Trigger: registry tvrdí `EmbedDimensions=3072`, ale provider/model vrátí jiný rozměr (změna verze modelu, špatná konfigurace) → zápis do fixního `halfvec(3072)` sloupce selže nebo (horší) tiše ořízne/mismatchne prostor s dotazovými embeddingy. · Očekávané chování: gateway validuje skutečný rozměr odpovědi vs deklarovaná dimenze PŘED zápisem do indexu; mismatch → `BusinessRuleException` `rag.embed_model_dimension_mismatch` (EC-30-11-01 sdílený errorCode), žádný tichý zápis do indexu nekonzistentního prostoru. · Mechanismus: +EC-30-11-01 dimenze guard + CONVENTIONS §3 `halfvec(3072)` fixní; oblast 03 embed. · Severity: P1 · Test: integration — fake embed vrátí 1536 dim pro 3072-deklarovaný model → reject, žádný insert do `hybridrag_chunks`.
