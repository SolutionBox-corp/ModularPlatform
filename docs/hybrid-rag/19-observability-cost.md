...
## Doplňky / Opravy z PDF audit (PDF §8 — cost po verzi promptu/configu)

### Doplněk k UC-19-03 (per-tenant nákladová atribuce)
- **EC-19-03-09 — Cost neatribuovatelný na verzi promptu/configu (chybí dimenze `prompt_version`/`config_version`)** · Trigger: system prompt se změní z v1 na v2 (delší instrukce, víc retrieved kontextu), cena za dotaz vzroste o ~30 %, ale `platform.rag.tokens` counter ani `rag_usage_daily` rollup nenesou verzi promptu/configu → nelze odpovědět „od v2 promptu vzrostla cena o 30 %" · Očekávané chování: token counter + rollup řádek nesou low-cardinality dimenze `prompt_version` (hash/semver system promptu + cache layout dle prompt-caching) a `config_version` (applied RagSetting snapshot, viz oblast 25 `AppliedConfigVersion`); verzí je málo → bezpečné na metrice; umožní groupBy + regresní srovnání cost před/po změně · Mechanismus: +Observability/cost; `platform.rag.tokens` tag `prompt_version`/`config_version` + rollup sloupce; cross-link 0-core UC-CORE-04 (ledger dimenze) + oblast 25 UC-25-03 AppliedConfigVersion · Severity: P1 · Test: integration — dva dotazy se shodným kontextem, prompt v1 vs v2 → assert ledger/counter rozlišuje `prompt_version`; cost report groupBy=prompt_version ukáže nárůst.

### Doplněk k UC-19-06 (nákladový report)
- **OPRAVA (UC-19-06): groupBy enum `model|day|stage` neumožní cost regresi po změně promptu → správně rozšířit o `prompt_version` a `config_version`** · Mechanismus: `GetRagCostReportQueryHandler` agreguje i přes dimenze `prompt_version`/`config_version` z `rag_usage_daily`; `GetRagCostReportValidator` přidá hodnoty do allowlistu groupBy.
- **EC-19-06-07 — Cost report groupBy=prompt_version pro regresní analýzu „před/po nasazení v2"** · Trigger: admin chce „kolik stála feature X před a po nasazení promptu v2" · Očekávané chování: `GET /v1/rag/admin/cost?groupBy=prompt_version` (resp. `config_version`) vrátí breakdown nákladu per verze v rozsahu; tenant-scoped z tokenu (EC-19-03-04 platí), neoceněné tokeny zvlášť (EC-19-06-04) · Mechanismus: +Observability/cost; rollup dimenze z EC-19-03-09; `IReadDbContextFactory` read nad `rag_usage_daily` · Severity: P2 · Test: integration — usage napříč v1/v2, groupBy=prompt_version → dva řádky se správnými součty + tenant izolace.
