...
## Doplňky / Opravy z PDF audit (PDF §6 — structured output / tool strict mode)

Doplnění k UC-CORE-14 (structured-output validace + tokenizer):

- **EC-CORE-14-08 — Structured output přes assistant-prefill → 400** · Trigger: gateway/konzument vynucuje JSON tím, že přidá trailing `role:"assistant"` zprávu s otevřeným `{` · Očekávané chování: na Opus 4.6+/4.8/Fable 5 vrací **400** (ne tiché ignorování staré semantiky); `ILlmGateway` MUSÍ strukturovaný výstup řešit přes `responseFormat` → `output_config.format`, NIKDY přes prefill ani trailing assistant turn. · Mechanismus: structured-output cesta gateway (validate→repair→fail, EC-CORE-14-01); provider adaptér nepoužívá assistant-prefill · Severity: P1 · Test: prefill request → fake provider 400; gateway cesta používá output_config.format.
- **EC-CORE-14-09 — `strict:true` umístění na tool definici (ne `tool_choice`)** · Trigger: konzument chce schema-strict tool input · Očekávané chování: gateway emituje `strict: true` jako TOP-LEVEL pole tool definice (vedle `name` + `input_schema`), NE do `tool_choice`; podmínkou je `additionalProperties:false` + vyplněné `required` ve schématu, jinak provider strict odmítne / nevynutí. · Mechanismus: tool serializace v provider adaptéru; schema preconditions ověřeny před callem · Severity: P1 · Test: strict tool → input bez extra polí; strict omylem v tool_choice → není vynuceno (regression test).
- **EC-CORE-14-10 — JSON schema constrainty nejsou model-enforced** · Trigger: schéma používá `minLength`/`maxLength`/`minimum`/`maximum`/`pattern`/rekurzivní `$ref` · Očekávané chování: provider tyto pokročilé constrainty NEvynucuje na úrovni modelu (vynutí jen tvar/typy/`required`/`enum`) — gateway MUSÍ po parsu provést **server-side re-validaci** přes JsonSchema.Net (máme ji); porušení → repair/retry/fail jako u nevalidního výstupu, NE tiše vrátit out-of-range hodnotu. · Mechanismus: JsonSchema.Net plná validace po deserializaci (nespoléhat na provider-side strict) · Severity: P1 · Test: schéma `maximum:10`, model vrátí 99 → server-side re-validace flagne → repair/fail, ne propuštění.
- **EC-CORE-14-11 — Verzování response schématu (V1/V2, ne mutace)** · Trigger: konzument potřebuje přidat/změnit pole strukturovaného výstupu · Očekávané chování: response schéma je **typed record verzovaný** (`AnswerV1`/`AnswerV2`), existující se NEMUTUJE — nová pole VŽDY nullable (zpětná kompatibilita pro staré klienty + cachované prefixy se schema-hash klíčem, EC-CORE-07-04); breaking změna = nová verze, ne přepis. · Mechanismus: schema hash v cache klíči; aditivní nullable pole; verzovaný record · Severity: P2 · Test: přidání nullable pole → starý klient parsuje dál; změna typu existujícího pole → nová verze schématu, ne in-place mutace.


---

## Doplňky / Opravy z PDF audit (PDF §8 — ledger verze + platform klíče)

### Doplněk k UC-CORE-04 (`AiUsageLedger`)
- **EC-CORE-04-08 — Ledger nenese `prompt_version`/`config_version` → cost neatribuovatelný na verzi promptu** · Trigger: `AiUsageEntry` má `feature`/`model`/4 countery, ale ne verzi promptu ani applied config, kterou volání použilo; po změně system promptu nelze rozdělit cost před/po (cross-ref oblast 19 EC-19-03-09) · Očekávané chování: ledger řádek nese volitelné `prompt_version` (hash nebo semver system promptu + cache layout dle UC-CORE-13) a `config_version` (applied config snapshot); append-only, immutable (zamražené při zápisu jako cost — EC-CORE-03-06); umožní agregaci „cost feature X za prompt v2" + regresní srovnání · Mechanismus: +AI gateway/cost; append-only ledger sloupce (vazba na UC-CORE-03 cost capture + UC-CORE-05 atribuce); konzument cost reportu (oblast 19) čte tyto dimenze · Severity: P1 · Test: integration — dvě volání s prompt_version v1 vs v2 → ledger rozlišuje; agregace groupBy prompt_version sedí.

### Doplněk k UC-CORE-16 (secrets / provider klíče)
- **OPRAVA (UC-CORE-16): EC-CORE-16-06 řeší jen budoucí „tenant chce vlastní klíč (BYOK)", ale neuvádí, že DNEŠNÍ stav (platform klíč pro všechny tenanty) je vědomé rozhodnutí s cross-tenant blast-radius → správně doplnit EC-CORE-16-07 (dnešní model + blast-radius) vedle EC-CORE-16-06 (BYOK = otevřené)** · Mechanismus: dokumentace dnešního stavu + blast-radius, ne jen „BYOK někdy".
- **EC-CORE-16-07 — HybridRag/platforma volá OpenAI/Cohere/Anthropic JEDNÍM platform klíčem pro všechny tenanty (ne BYOK) — vědomé rozhodnutí + blast-radius** · Trigger: retrieval/syntéza volá OpenAI (embed), Cohere (rerank), Anthropic (chat) přes platform `Ai:*:ApiKey` sdílený napříč všemi tenanty · Očekávané chování: zdokumentovat jako VĚDOMÉ rozhodnutí, ne implicitní stav. Blast-radius: (a) kompromitace platform klíče zasáhne všechny tenanty; (b) provider-side rate-limit a spend jsou sdílené → jeden tenant může vyčerpat kvótu/rozpočet ostatním (per-tenant budget UC-CORE-06 mitiguje náš spend, ale provider 429 zůstává sdílený); (c) PII všech tenantů teče k sub-procesorům pod jednou identitou (cross-link oblast 16 UC-16-11 PII egress). Per-tenant BYOK = OTEVŘENÉ (EC-CORE-16-06): pokud se zavede, klíč `[Encrypted]` per-tenant + izolovaný rate-limit/spend bucket · Mechanismus: +AI gateway/secrets; CONVENTIONS secrets Options + `[Encrypted]` (pro budoucí BYOK); vazba na UC-CORE-06 budget + oblast 16 UC-16-11 · Severity: P2 · Test: dokumentace egressu existuje; ledger atribuuje spend per tenant i při sdíleném klíči (UC-CORE-05); (BYOK až bude — klíč šifrován + izolovaný bucket).
