...
## Doplňky / Opravy z PDF audit (PDF §6 — citace × structured output, mid-conversation system)

- **EC-13-06-06 — Native Anthropic Citations vs `output_config.format` (nekompatibilní)** · Trigger: někdo zapne nativní Anthropic Citations API SOUČASNĚ se strukturovaným výstupem (`responseFormat=json` / `output_config.format`) · Očekávané chování: tato kombinace je **nekompatibilní → 400**. Katalog používá APLIKAČNÍ citace (model vrací `chunkRef`, `CitationResolver` je mapuje na `Chunk.Id`) — to je správně a se structured output funguje; nativní Citations se zde NESMÍ zapínat. · Mechanismus: aplikační citation resolve (UC-13-01/06), nikdy nativní Citations + JSON schema zároveň · Severity: P1 · Test: pokus zapnout nativní Citations + JSON format → fake gateway 400; produkční cesta používá aplikační chunkRef.
- **EC-13-06-07 — Verzování citation/answer schématu** · Trigger: rozšíření strukturované odpovědi o nové pole (např. `confidence`) · Očekávané chování: schéma odpovědi je verzovaný typed record (`AnswerResponseV1`/`V2`), existující se NEMUTUJE; nová pole nullable → staří klienti i schema-hash cache klíč zůstanou validní; breaking změna = nová verze schématu/endpointu. · Mechanismus: aditivní nullable pole, verzovaný record (cross-ref EC-CORE-14-11) · Severity: P2 · Test: přidání nullable pole → starý klient OK; změna typu existujícího pole → nová verze.
- **EC-13-01-10 — Citation-policy instrukce nesmí být imperativně „křičící"** · Trigger: system prompt obsahuje „CRITICAL: YOU MUST ALWAYS cite every sentence" · Očekávané chování: modely 4.x na agresivní imperativ **přestřelují** — citují i triviální/konverzační věty, fabrikují citace aby splnily „MUST" (zhoršuje groundedness, ne zlepšuje). Formulovat podmínkově: „Cite a source when stating a fact drawn from the documents." Tvrdou záruku stejně drží deterministický `CitationGuard` v kódu (UC-13-02), ne formulace promptu. · Mechanismus: podmínkové znění citation policy ve stabilním (cachovaném) system prefixu; out-of-band guard jako autorita · Severity: P2 · Test: prompt „YOU MUST cite every sentence" → over-citace/fabrikace na konverzačních větách; podmínkové znění → guard flagne nepodložené, žádná fabrikace.
- **EC-13-15-06 — Mid-conversation závazná instrukce jako system role** · Trigger: v multi-turn konverzaci je třeba změnit chování (např. „dál odpovídej jen z kolekce X") · Očekávané chování: instrukce se posílá jako `{role:"system"}` zpráva do `messages[]` (Opus 4.8), NE editací top-level `system` (busti cachovaný synthesis prefix, UC-13-10) ani vložením do user textu (nedůvěryhodné — indirect injection ji nerozliší od dat). · Mechanismus: top-level system = turn-invariant cachovaný; dynamická instrukce = system-role zpráva v konverzační historii za breakpointem · Severity: P2 · Test: mid-conversation instrukce → top-level system beze změny (cache hit), instrukce je system-role, ne user.
