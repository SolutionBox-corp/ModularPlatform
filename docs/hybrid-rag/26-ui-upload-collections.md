...
- **EC-26-01-06 — Drop mimo dropzone (browser navigace + ztráta fronty)** · Trigger: uživatel pustí soubor(y) vedle dropzone / kamkoli do okna prohlížeče · Očekávané chování: window-level `dragover`/`drop` `preventDefault` — prohlížeč NEodnaviguje na otevření souboru (default), upload fronta + rozdělaný stav se neztratí; mimo dropzone se nic nezařadí, jen se potlačí default · Mechanismus: global drag/drop preventDefault (web standard) + Surrounding Concerns (no data loss) · Severity: P1 · Test: drop souboru mimo dropzone → stránka zůstane, 0 navigací, fronta zachována.
- **EC-26-01-07 — Cizí / cross-tenant collectionId v upload route (IDOR)** · Trigger: ručně podstrčené `collectionId` jiného uživatele/tenanta do BFF multipart POST · Očekávané chování: backend 404 (RLS/Scope), žádný osamělý blob ani `Document`; UI not-found, ne leak existence kolekce (odlišeno od read-only kolekce EC-26-01-05) · Mechanismus: oblast 16 izolace + zákon 10 (scope z tokenu, ne z těla) · Severity: P0 · Test: POST s cizím collectionId → 404, 0 blobů ve storage.
- **EC-26-01-08 — Spoofnutý content-type (přípona/MIME ≠ skutečné bajty)** · Trigger: spustitelný soubor přejmenovaný na `.pdf` s podvrženou hlavičkou `application/pdf` · Očekávané chování: klientské zrcadlo allowlistu projde (zná jen deklarovaný typ), ale backend sniffuje magic bytes a odmítne `rag.document.unsupported_type`; UI řádek Rejected z backend chyby · Mechanismus: backend content sniffing = autorita (oblast 01, Files `UploadFileValidator`) · Severity: P1 · Test: EXE bajty jako `.pdf` → backend 400, řádek Rejected, žádná ingest saga.
- **EC-26-01-09 — Prázdný / 0-bajtový soubor** · Trigger: drop souboru s nulovou velikostí · Očekávané chování: klient řádek Rejected „prázdný soubor"; i při obejití backend odmítne, žádný junk `Document` + okamžitě failující saga · Mechanismus: klient size>0 check + backend validator (oblast 01) · Severity: P1 · Test: 0 B soubor → Rejected, 0 ingest saga.
- **EC-26-01-10 — Síťový disconnect uprostřed přenosu bajtů** · Trigger: spojení spadne během uploadu (ne cancel, ne 5xx) · Očekávané chování: řádek přejde na `UploadFailed` (NE „forever Uploading"), progress zmrazí + nabídne retry (UC-26-03); pokud backend přesto `Document` založil, ukáže se po refetchi listu · Mechanismus: fetch error handling + UC-26-03 retry; refetch jako zdroj pravdy · Severity: P1 · Test: kill síť mid-transfer → řádek UploadFailed s retry, ne zaseknuté Uploading.
