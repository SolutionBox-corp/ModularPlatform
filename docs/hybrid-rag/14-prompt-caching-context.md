...
- **EC-14-01-08 — PII chunk-content v cachovaném prefixu u Anthropic (sub-processor + post-erasure residual)** · Trigger: retrieved-docs blok (C) je v CACHOVANÉ části (před breakpointem) a obsahuje `[Encrypted][PersonalData]` dešifrovaný Content; Anthropic prompt cache drží prefix po TTL (5 min / 1 h) — tedy plaintext PII je dočasně u externího processoru i po GDPR erasure subjektu, dokud cache nevyexpiruje · Očekávané chování: tato expozice musí být VĚDOMĚ ohraničená a zdokumentovaná (analog zákona „Durable-envelope PII bound" — Wolverine envelope PII má `KeepAfterMessageHandling`); buď akceptovat TTL-bounded residual jako přijatelný, nebo nevkládat PII do cachovaného prefixu (jen do volatilní části za breakpointem, kde se necachuje), případně extended-TTL vypnout pro PII korpusy · Mechanismus: rozhodnutí o umístění PII vůči breakpointu; vazba na UC-16-11 (sub-processor egress) · Severity: P1 · Test: po erase subjektu žádný NOVÝ dotaz neposílá jeho PII; dokumentován max TTL residualu; volba breakpoint-vs-PII otestována.


---

## Doplňky / Opravy z PDF audit (PDF §6 — prompt caching / structured output)

### Opravy anatomie promptu a cache prahů

- **OPRAVA (UC-14-01): main flow bod 2 řadí prefix (A) system → (B) tool definitions → (C) retrieved-docs → správně (A) tool definitions → (B) system → (C) retrieved-docs.** Anthropic Messages request se renderuje v pořadí **tools (pozice 0) → system → messages[]**, tedy `tools` jsou NEJVÍC upstream, ne system. Cachovaný prefix proto musí být tools → system → retrieved-docs → breakpoint → volatilní suffix. · Mechanismus: cache prefix match jde shora dolů přesně v pořadí serializace requestu; když je v katalogu system před tools, breakpoint za system necachuje tools (jsou v jiném, výše umístěném poli) a posune se match.
- **OPRAVA (UC-14-04): bod 3 „umisťuje breakpointy od nejstabilnějšího (system) po nejvolatilnější" → nejvíc upstream NENÍ system, ale tool definitions.** Breakpointy se kladou od nejvíc upstream segmentu: **tools → system → retrieved-docs**. Důsledek: jakákoli změna tool setu (přidání/odebrání/reorder nástroje, změna schématu) busti VEŠKERÝ následující cache (system + docs), protože tools jsou prefix všeho ostatního. · Mechanismus: kumulativní prefix match; tools nejvíc upstream.
- **OPRAVA (EC-14-01-03): „min cacheable threshold 1024/2048 tokenů" je nepřesné → minimum je per-model.** Opus 4.8 = **4096** tokenů, Sonnet 4.6 = **2048** tokenů. Hodnota MUSÍ být per-model lookup (`Rag:Prompt:MinCacheablePrefixTokens` keyed modelem), ne jedna konstanta. Pod prahem Anthropic prefix tiše neuloží → `cache_creation_input_tokens = 0` i `cache_read = 0`, bez chyby — nutno detekovat metrikou (UC-14-07), ne předpokladem že cache běží. · Mechanismus: model capability lookup; tichý nemiss pod prahem.

### Nové edge cases

- **EC-14-01-09 — Assistant-prefill pro vynucení JSON → 400 (mrtvý pattern)** · Trigger: assembler ukončí `messages[]` blokem `role:"assistant"` s otevřeným `{` aby „donutil" model vrátit JSON · Očekávané chování: na Opus 4.6+/4.8/Fable 5 to NENÍ tiše ignorováno — vrací **400**; prefill se NESMÍ používat. Strukturu vynucuj VÝHRADNĚ přes `output_config.format` (resp. MEAI `ChatResponseFormat`/`responseFormat`), nikdy přes trailing assistant turn. · Mechanismus: structured-output cesta gateway (UC-CORE-14); assembler nikdy nepřidává trailing otevřený assistant blok · Severity: P1 · Test: assembler s prefill → fake gateway 400; assert produkční cesta používá output_config.format, ne assistant prefill.
- **EC-14-01-10 — Závazná instrukce vzniklá uprostřed běžící konverzace** · Trigger: během konverzace je třeba vnutit novou závaznou instrukci (např. „od teď cituj jen z kolekce X") · Očekávané chování: poslat ji jako `{role:"system"}` zprávu do `messages[]` (Opus 4.8 podporuje mid-conversation system role), NIKDY (a) editovat top-level `system` blok (busti cachovaný prefix → 0 % hit), ani (b) vložit do user/tool textu (spoofovatelné — indirect injection ji nerozliší od dat). · Mechanismus: top-level `system` zůstává turn-invariant (cachovaný); dynamická instrukce = system-role zpráva ve volatilním suffixu za breakpointem · Severity: P1 · Test: mid-conversation instrukce → assert top-level system beze změny (cache_read>0) a instrukce je system-role v messages, ne user text.
- **EC-14-01-11 — Few-shot příklady: stabilní → system, dynamicky vybírané → za breakpoint** · Trigger: prompt obsahuje few-shot příklady · Očekávané chování: STABILNÍ (fixní, vždy stejné) few-shoty patří do `system` bloku (cachovaný prefix, zaplatí se jednou); příklady DYNAMICKY vybírané per-query (retrieval/podobnost dotazu) MUSÍ za breakpoint do volatilní části — jinak každý dotaz s jiným výběrem busti prefix. · Mechanismus: oddělení turn-invariant few-shot (prefix) od per-query few-shot (suffix), stejná hranice jako retrieved-docs · Severity: P2 · Test: dva dotazy s různými dynamickými few-shoty → cachovaný prefix byte-identický (cache_read>0); statické few-shoty součást prefixu.
- **EC-14-04-05 — Změna tool setu busti celý následující cache** · Trigger: deployment přidá/odebere nástroj nebo změní pořadí/schéma; tools jsou nejvíc upstream segment · Očekávané chování: protože breakpointy jdou tools → system → docs, jakákoli změna tools invaliduje VŠECHNY navazující breakpointy (legitimní, ne chyba) — `toolsVersion` v metrice odhalí přebudování; pořadí nástrojů MUSÍ být deterministické (stabilní serializace), aby se prefix nebustil bez skutečné změny. · Mechanismus: deterministické řazení tools + `toolsVersion`; tools nejvíc upstream (oprava UC-14-04) · Severity: P1 · Test: reorder tools beze změny obsahu → stejný prefix hash; přidání tool → nový prefix, žádná chyba.
- **EC-14-06-05 — 20-block breakpoint lookback v agentic tool loopu** · Trigger: agentic RAG roste tool_use/tool_result bloky; breakpoint je až za >20 bloky historie · Očekávané chování: Anthropic hledá cache match jen **max 20 bloků zpět** od breakpointu — přesah = **TICHÝ cache miss** (žádná chyba, jen `cache_read=0`); assembler proto MUSÍ udržovat intermediate breakpoint zhruba **každých 15 bloků** (rolling), aby match nikdy nepřesáhl okno. · Mechanismus: rolling intermediate breakpoint ≤15 bloků; ≤4 breakpointy celkem (UC-14-04) → nejstarší breakpoint recyklovat · Severity: P1 · Test: tool loop > 20 bloků bez intermediate breakpointu → cache_read padá na 0; s rolling breakpointem zůstává > 0.
- **EC-14-06-06 — Agresivní prompting overtriggeruje nástroje na 4.x** · Trigger: tool/system instrukce typu „CRITICAL: YOU MUST call search_knowledge" · Očekávané chování: modely 4.x na imperativní/„křičící" formulace **přestřelují** — volají nástroj i tam, kde nemá smysl (zbytečné tool cally = cost + latence). Formulovat podmínkově: „Use search_knowledge when the question references the user's documents." · Mechanismus: tool description + system guidance podmínkové, ne imperativní; stabilní (cachované) znění · Severity: P2 · Test: dotaz nevyžadující retrieval → s „YOU MUST" zněním tool zavolán zbytečně; s „use when" zněním ne.
- **EC-14-06-07 — `strict:true` je pole TOOL definice, ne `tool_choice`** · Trigger: assembler chce vynutit, aby tool input přesně seděl na schéma · Očekávané chování: `strict: true` je TOP-LEVEL pole na DEFINICI nástroje (vedle `name`/`input_schema`), NE součást `tool_choice` (častý omyl); vyžaduje, aby `input_schema` mělo `additionalProperties: false` a vyplněné `required`. · Mechanismus: gateway nastavuje strict na tool definici; schema preconditions (cross-ref UC-CORE-14) · Severity: P2 · Test: tool s `strict:true` + `additionalProperties:false` → model nevrací extra pole; chybné umístění do tool_choice → schéma se nevynutí (regression).
