...
- **EC-32-13-03 — Saga row (PendingEffect / payload) jako PII-persistence kanál přežívající erasure** · Trigger: `ApprovalGateSaga` perzistuje serializovaný `PendingEffect` command + payload (UC-32-04/13) v saga tabulce; ten může nést PII (odpověď, dotaz uživatele, jméno) a saga row čeká dny; saga tabulka NENÍ `[Encrypted]`/`IDataSubject` → po GDPR erasure subjektu PII v saga rowu přežije. · Očekávané chování: PII v PendingEffect/payload je šifrovaná (`[Encrypted]`+`[PersonalData]`, crypto-shred po erasure) NEBO má saga bounded lifetime (timeout/abandon) jako durable-envelope PII bound; saga nesmí být tichý kanál přežívající erasure. · Mechanismus: +oblast 20 crypto-shred + CLAUDE.md „Durable-envelope PII bound" + EC-32-01-03 (payload PII at-rest); rozhodnout analogicky CONVENTIONS §12. · Severity: P0 · Test: integration — gate s PII v PendingEffect čeká → erasure subjektu → saga PII `[erased]`/nedostupné, ne plaintext v saga rowu.
- **EC-32-04-05 — Zastaralý PendingEffect přehrán proti změněnému světu bez re-validace preconditions** · Trigger: schválení dorazí za hodiny/dny (UC-32-13); mezitím se cílový stav změnil — kolekce smazána, entity zaniklá, subjekt erased, kvóta nyní vyčerpaná → saga slepě dispatchne uložený command. · Očekávané chování: PendingEffect command re-validuje preconditions PŘI provedení (cílová entita existuje, scope/permission stále platí, kvóta) — ne blind replay; neplatné preconditions → operace `Failed` s důvodem, ne provedení proti zmizelému/změněnému cíli. · Mechanismus: +UC-32-04 saga resume + cílový command vlastní validace/guardy (ValidationBehavior + business rule); idempotency UNIQUE key (EC-32-04-02). · Severity: P1 · Test: integration — gate na delete kolekce → kolekce mezitím smazána → approve → no-op/Failed, ne crash ani duplicitní efekt.
- **EC-32-04-06 — Non-gated side-effekty proběhly, gated efekt deny → částečný nekonzistentní stav bez kompenzace** · Trigger: UC-32-04 krok 3 nechá ne-gated side-effekty proběhnout a zadrží jen gated; při `deny` už ne-gated efekty (uložené citace, částečně servírovaná data, externí call) zůstávají bez kompenzace. · Očekávané chování: hranice gated vs ne-gated je explicitní a bezpečná — buď je celá citlivá akce gated (nic se neprovede před schválením), nebo ne-gated efekty jsou idempotentně kompenzovatelné/bezvýznamné při deny; žádný nekonzistentní polostav vydaný jako fakt. · Mechanismus: +Zákon 2 (commit je atomický) + saga kompenzace přes idempotentní commandy (CLAUDE.md saga — kompenzace ne přes výjimky); oblast 04. · Severity: P1 · Test: integration — gated akce deny → žádný trvalý částečný efekt zůstal nekompenzovaný.
- **EC-32-11-04 — Odebraná sensitive permission se neprojeví (token snapshot) → revokovaný approver stále schválí** · Trigger: admin odebere reviewerovi `RagReviewSensitive`, ale jeho token nese starý claims snapshot (opak EC-32-11-02); reviewer s živým tokenem decide legal/irreversible item → handler RBAC čte claim z tokenu → stále povoleno do expirace/re-auth. · Očekávané chování: pro citlivé/irreversible kategorie se RBAC ověřuje proti aktuálnímu stavu (per-call DB/claim re-check jako MCP výjimka EC-15-06-05), nebo má krátkou token TTL; revokovaný approver nesmí schvalovat sensitive akce na základě stale snapshotu po revoke. · Mechanismus: +CLAUDE.md authorization (claims snapshot) + CONVENTIONS §13 (per-call check jako výjimka pro citlivé); separation-of-duty / freshness pro `RagApproveIrreversible`. · Severity: P1 · Test: integration — revoke `RagReviewSensitive` → starý token decide legal item → 403 (ne tiché povolení do expirace).
- **EC-32-10-03 — Eskalace/routing na prázdnou reviewer skupinu → item zaseknutý navždy** · Trigger: policy směruje item na role/team, který nemá žádné aktivní členy (všichni smazáni/erased), nebo eskalace (UC-32-08) míří do nejvyššího stupně bez obsazení; EC-32-10-01 řeší jen named→team fallback, ne prázdný terminální set. · Očekávané chování: prázdná/nedosažitelná reviewer skupina → fallback na vyšší/default eskalační stupeň + WARN + `review_queue_depth`/stuck alert; po `ApprovalTimeoutHours` saga timeout terminalizuje (abandon/Failed dle policy); item nikdy nezůstane Pending navždy bez upozornění. · Mechanismus: +UC-32-08 eskalace + EC-32-04-03 saga timeout + EC-32-18-02 stuck-queue alert (vzor `MessagingHealthJob`). · Severity: P1 · Test: integration — routing na prázdný team → WARN/alert + po timeoutu terminalizace, ne věčný Pending.
- **EC-32-05-03 — Serve-with-flag: odpověď servírována, ale enqueue review selže → answer bez review** · Trigger: async band (UC-32-05) servíruje odpověď uživateli okamžitě a souběžně enqueue `RagReviewItem`; pokud enqueue/commit review položky selže po odeslání odpovědi, flagged odpověď je venku bez záruky posouzení. · Očekávané chování: uložení pending-review turnu a enqueue review položky je ATOMICKÉ (jeden outbox commit) — buď je odpověď označena pending-review A review item existuje, nebo se odpověď neoznačí jako flagged-but-unreviewed; žádná flagged odpověď bez odpovídajícího review item. · Mechanismus: +Zákon 2 (outbox commit = `SaveChangesAndFlushMessagesAsync` atomicky) + UC-32-01 enqueue ve stejné transakci jako turn flag. · Severity: P1 · Test: integration — simuluj fail enqueue → turn se nestane „flagged bez review item" (rollback nebo není flagged); invariant: každý serve-with-flag turn má review item.
- **EC-32-16-05 — Feedback flood (masový thumbs_down) → amplifikace review queue / DoS** · Trigger: uživatel pošle negativní feedback na stovky/tisíce vlastních distinct queries; každý low-score thumbs_down routuje nový `RagReviewItem` (UC-32-16 krok 3) → fronta zahlcena (EC-32-16-02 řeší jen per-query idempotenci, ne objem napříč queries). · Očekávané chování: feedback endpoint je per-user rate-limited (oblast 22 request-edge hardening) a routing do review je dedup/agregovaný + gated samplingem/skóre, takže jeden uživatel nemůže amplifikovat frontu do DoS; legitimní feedback se uloží, jen review-routing je tlumený. · Mechanismus: +oblast 22 rate-limit (`auth`-style per-user policy) + UC-32-06 sampling/threshold gate na routing do review; EC-32-18-02 queue-depth alert. · Severity: P1 · Test: integration — 1 uživatel 1000× thumbs_down → feedback řádky uložené, ale review queue není zahlcena 1000 items (rate-limit/dedup).
