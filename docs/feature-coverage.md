...
| Account does not yet exist when top-up arrives (event ordering) | ✓ | Inline create-on-miss with race recovery (CreditTopUpHandler.cs:28-42): on UNIQUE collision detaches and reloads the winner's account. |
...
**Test gaps:** No test asserts the non-expiring bucket (BucketExpiryDays null -> ExpiresAt null) is created and that such a bucket is never picked up by the expiry sweep; No test for the inline create-on-miss path inside CreditTopUpHandler when no account pre-exists (the auto-provision event normally wins first); the create-then-UNIQUE-collision recovery branch (CreditTopUpHandler.cs:36-41) is untested
...
**Testy:** NotificationsIntegrationTests.SendNotification_persists_an_inapp_row_and_enqueues_channel_delivery_via_the_outbox (NT-1); NotificationsIntegrationTests.SendNotification_with_missing_template_key_returns_not_found; TemplateRendererTests.SendNotificationCommand_carries_channels_and_data
**Test gaps:** No test asserts an EmailDeliveryRequested/PushDeliveryRequested message actually landed in wolverine_outgoing_envelopes (NT-1 infers the outbox only from the 200 + persisted row); No test for the Distinct() de-dup of channels; No test that a blank ToAddress email channel is a no-op in the Worker (EmailDeliveryHandler short-circuit); No test for locale fallback (locale='cs' with only an 'en' template row)
...
| Transition on a missing operation id | ✓ | OperationStore.cs:47-48 throws NotFoundException('operation.not_found') if the row is absent |
...
**Testy:** OperationsTests.A_terminal_operation_is_not_resurrected_by_a_duplicate_worker_transition; OperationWorkerFailureTests.Worker_marks_the_operation_failed_when_the_work_throws
**Test gaps:** No test of MarkRunning on an already-Failed/Succeeded op via the worker path under concurrency; No test that a NotFoundException from TransitionAsync (operation deleted) is surfaced/retried sanely; No stuck-Pending detection/reaper test — there is no job that finds operations stuck Pending/Running if a message was permanently dead-lettered (relies entirely on the catch terminalizing)
...
| Bytes written to storage but metadata SaveChanges fails (orphan blob) | ✗ | UploadFileHandler.cs:22 PutAsync runs first, then db.SaveChangesAsync at line 35 with no compensating storage.DeleteAsync on failure → an orphaned blob is left in storage if the DB write throws. No outbox/transaction spans storage. Low impact (key is unreferenced, GDPR erase won't find it since no row), but it is a real orphan/leak gap. |
...
**Testy:** FilesUploadTests.Upload_then_download_round_trips_the_same_bytes_and_content_type; FilesUploadTests.Disallowed_content_type_is_rejected; FilesUploadTests.Oversized_file_is_rejected; FilesUploadTests.Upload_location_header_is_versioned_and_points_at_the_download_route
**Test gaps:** No test for the orphan-blob path (PutAsync succeeds, metadata write fails); No test for empty/missing content-type behavior; No test that the storage key is NOT derived from the client filename (asserted only by code/comment)

_Strong security posture. The only genuine gap is no compensation if metadata persistence fails after the blob is written (orphan blob)._
...
| GetAsync on a missing key | ◐ | LocalFileStorage.cs:37 throws FileNotFoundException; S3 throws AmazonS3Exception — these are provider-native exceptions that the download endpoint does not translate to 404 (see download feature) |
...
**Testy:** StorageUnitTests.Invalid_storage_keys_are_rejected; StorageUnitTests.Opaque_keys_are_accepted; StorageUnitTests.S3_config_for_minio_uses_service_url_and_path_style; StorageUnitTests.S3_config_for_real_s3_uses_region_not_service_url
...
| Edge case | | Jak se k tomu stavíme |
|---|:--:|---|
| Headers set before next() so they apply even on error responses | ✓ | Headers written first, then await next() (SecurityHeadersMiddleware.cs:10-15); runs before GlobalExceptionMiddleware in the pipeline (PlatformWebExtensions.cs:114-123) |
...
**Test gaps:** No test asserts the four security headers are present on a response (trivial integration assertion, currently absent)

_Correct and minimal; only gap is the total absence of a test asserting the headers._
