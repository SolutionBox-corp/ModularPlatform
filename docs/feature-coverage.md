...
| Subscription canceled locally but reactivated in Stripe | ✓ | Pass 2 scans every local subscription mirror, including `Canceled`, so Stripe object state can reactivate a row through the same `UpsertSubscriptionFromStripeCommand` path as webhooks. Proven by `StripeReconcileTests.Reconcile_reactivates_a_locally_canceled_subscription_when_stripe_is_active_again`. |
...
_All three passes have focused behavioral coverage including cap branches, canceled-to-active drift, and the drift metric export._
...
**Nekonzistence v oblasti (1):**
- Reconcile drift scope now includes local Canceled subscription mirrors too, so a row reactivated in Stripe converges back through the same upsert path as webhooks.
...
| Ring buffer over capacity | ✓ | LocalRealtimePublisher UserBuffer trims front when > MaxEvents (Realtime.cs:250-261); Redis uses exact MAXLEN (Realtime.cs:119-123). Local and Redis exact eviction are both tested. |
...
| Approximate MAXLEN means Redis may retain MORE than MaxEvents | ✓ | Redis replay now uses exact MAXLEN (`useApproximateMaxLength:false`), so MaxEvents is an exact count bound for both local and Redis replay. Proven by RedisRealtimeReplayTests.Redis_replay_trims_the_stream_exactly_to_max_events. |
| Local ring id overflow / cross-restart cursor | ✓ | LocalRealtimePublisher detects a valid numeric cursor greater than the newest current-process id and treats it as a stale process-lifetime cursor, replaying the current bounded buffer instead of returning nothing; proven by RealtimeReplayTests.Local_replay_treats_a_future_cursor_as_stale_process_lifetime_and_replays_the_current_buffer. |

**Testy:** RedisRealtimeReplayTests.Redis_replay_reads_only_events_after_the_last_event_id_and_refreshes_ttl; RedisRealtimeReplayTests.Redis_replay_is_user_scoped_and_disabled_mode_does_not_store_a_stream; RedisRealtimeReplayTests.Redis_replay_trims_the_stream_exactly_to_max_events; RealtimeReplayTests.ReadSinceAsync_with_null_or_empty_lastEventId_returns_empty; RealtimeReplayTests.ReadSinceAsync_returns_only_events_newer_than_cursor; RealtimeReplayTests.ReadSinceAsync_at_last_event_returns_empty; RealtimeReplayTests.Local_replay_treats_a_future_cursor_as_stale_process_lifetime_and_replays_the_current_buffer; RealtimeReplayTests.Ring_buffer_bounded_to_maxEvents_evicts_oldest_first; RealtimeReplayTests.ReadSinceAsync_for_unknown_userId_returns_empty; RealtimeReplayTests.Events_from_different_users_are_isolated; RealtimeReplayTests.Disabled_replay_buffer_returns_empty_from_ReadSince; RealtimeReplayTests.Redis_stream_cursor_increment_handles_missing_malformed_and_overflow_sequence
**Test gaps:** No remaining focused replay-buffer gap in this slice. Redis and local count bounds are exact; TTL remains the hard PII time bound and is covered by a live Redis test.
...
| An in-Api background Wolverine handler assumed tenant-scoped | ✓ | HttpTenantContext treats absence of HttpContext as system and a real anonymous/authenticated HTTP request as non-system; pinned by HttpTenantContextTests.No_http_context_is_system_context_for_in_api_background_work and Anonymous_http_context_is_not_system_and_does_not_bypass_tenant_filters. |

**Testy:** TenantIsolationTests.Two_users_land_in_distinct_tenants_in_the_same_users_table; A_user_reading_through_the_tenant_filter_sees_only_their_own_tenant_data; Anonymous_caller_with_no_tenant_claim_is_rejected_not_granted_global_visibility; HttpTenantContextTests.No_http_context_is_system_context_for_in_api_background_work; HttpTenantContextTests.Anonymous_http_context_is_not_system_and_does_not_bypass_tenant_filters; HttpTenantContextTests.Authenticated_http_context_reads_user_tenant_and_ip_from_the_current_request; TenantStampingInterceptorTests.SavingChanges_does_not_overwrite_explicit_tenant_id; PlatformDbContextQueryFilterTests.Query_filters_apply_tenant_and_soft_delete_together; PlatformDbContextQueryFilterTests.System_context_bypasses_tenant_filter_but_keeps_soft_delete_filter
...
| No OTLP collector configured | ✓ | AddPlatformTelemetry fail-fasts outside Development/Testing unless an explicit OTLP endpoint is configured via OpenTelemetry:Otlp:Endpoint / OTEL_EXPORTER_OTLP_ENDPOINT, or both signal-specific endpoint env vars. Pinned by TelemetryBehaviorTests.Platform_telemetry_requires_an_explicit_otlp_endpoint_outside_development. |

**Testy:** JobFailureMetricsTests uses a MeterListener on the 'ModularPlatform' meter — indirectly proves the shared meter name + instrument export path; TelemetryBehaviorTests.ModularPlatformException_tags_activity_with_error_code; TelemetryBehaviorTests.Platform_registration_keeps_telemetry_behavior_outer_most; TelemetryBehaviorTests.Platform_telemetry_requires_an_explicit_otlp_endpoint_outside_development; TelemetryBehaviorTests.Platform_telemetry_accepts_a_configured_otlp_endpoint_outside_development
