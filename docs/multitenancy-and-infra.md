...
| 5 | **Registrace → join existujícího tenanta** (ne create-new) + **invite-gating** | ✅ **POSTAVENO** | Subdoména→join: `RegisterUserHandler` + `TenantResolutionMiddleware`. Gating: `Tenant.RegistrationMode` (Open / **InviteOnly = default** / Closed) + `tenant_invites` (single-use, SHA-256 token); port `ITenantRegistrationGate` (Identity→Tenancy, fail-closed); platform-admin `POST /tenant/admin/tenants/{id}/invites`. Apex signup = self-serve provision (negated). Migrace `20260612062343_TenantRegistrationGating` |
