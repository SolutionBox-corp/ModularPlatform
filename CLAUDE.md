...
| **Retention sweep** | `GdprRetentionSweepJob` → `RetentionSweepCommand`: purges shredded `subject_keys` tombstones past `Gdpr:Retention:ShreddedKeyRetentionDays` (default 30); counter `platform.gdpr.retention_swept`. Per-module data retention stays per-module (boundary law) — this is the pattern to copy | copy for module-owned retention | a cross-module reaper |
