...
| **Stuck-message / outbox reconciliation job** | base retry/DLQ is wired; no periodic stuck-outbox / dead-letter scan + alert | reconciliation cadence + alerting target |

(**Audit-PII encryption-at-rest is now SOLVED** — crypto-shred via `[PersonalData]`/`IDataSubject`/`IPersonalDataProtector`; see §4. Follow-ups, not blockers: bind `subjectId` as AES-GCM AAD; blank `PasswordHash` on erasure for credential hygiene.)
